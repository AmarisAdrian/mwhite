...
$id = $user_info->id;
$user = UserModel::repo()->find($user['id']); 
$username= $user->firstname;
$email=$user->email;
$account_stat=$user->role;
$user->date_creation;
$month = date('m');
$usera = UserModel::repo()->find($id); 
if(isset($_POST['savenote'])){
 ...
 $all_usera = UserModel::repo()->GetAll();

$staff_mem = 0;
$clients_mem = 0;
if (is_array($all_usera)) {
	foreach($all_usera as $all_user){
		$staffmemb = $all_user->role;
		if($staffmemb == 3 ){
			$staff_mem++;
		}
		if($staffmemb == 2 ){
...
<div class="total-pro"><?php //echo count($projects); ?>0<span><?php echo $lang['Total Projects']; ?></span></div>
</div>
<div class="container">
...
<textarea name="snote" class="snote" placeholder="<?php echo $lang['Write note Here!']; ?>"><?php echo $user->info;?></textarea>
</form>
</div>
     </div>
